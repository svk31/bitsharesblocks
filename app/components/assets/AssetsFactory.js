  ...
  function getSymbol(id) {
    if (id > 1) {
      if (_assets[id]) {
        ...
        return _forceRefresh().then(function() {
          return _assets[id].symbol;
        });
      }
    } else {
  ...
  function _assetsInfo(result, marketBoolean) {
    ...
    if (marketBoolean) {
      ...
      for (i = 0; i < assets.length; i++) {
        ...
        assets[i].dailyVolumeText = $filter('number')(assets[i].dailyVolume, 2) + ' ' + _baseAsset;

        assets[i].lastPrice = (assets[i].lastPrice !== 0) ? (1 / assets[i].lastPrice) : 0;
        assets[i].cap = (assets[i].lastPrice) * assets[i].current_supply;
        assets[i].capText = $filter('number')(assets[i].cap, 0) + ' ' + _baseAsset;
        assets[i].vwapText = $filter('number')((assets[i].vwap !== 0) ? 1 / assets[i].vwap : 0, 3) + ' ' + _baseAsset;
        assets[i].lastPriceText = $filter('number')(assets[i].lastPrice, 3) + ' ' + _baseAsset;
  ...
  function orderBook(asset) {
    ...
    if (asset.order_history.length > 0) {
      ...
      asset.order_history.forEach(function(order) {
        ...
        order.ask_paid.amount = $filter('number')(order.ask_paid.amount / basePrecision, 2) + ' ' + _baseAsset;
        order.ask_received.amount = $filter('number')(order.ask_received.amount / asset.precision, 3) + ' ' + asset.symbol;
        order.ask_index.order_price.ratio = $filter('number')(1 / (order.ask_index.order_price.ratio * priceRatio), 3) + ' ' + _baseAsset + '/' + asset.symbol;

        order.bid_paid.amount = $filter('number')(order.bid_paid.amount / asset.precision, 3) + ' ' + asset.symbol;
        order.bid_received.amount = $filter('number')(order.bid_received.amount / basePrecision, 2) + ' ' + _baseAsset;
        order.bid_index.order_price.ratio = $filter('number')(1 / (order.bid_index.order_price.ratio * priceRatio), 3) + ' ' + _baseAsset + '/' + asset.symbol;
      ...
      if (tradesFound) {
        asset.dailyLow = $filter('number')(asset.dailyLow, 2) + ' ' + _baseAsset + '/' + prefix + asset.symbol;
        asset.dailyHigh = $filter('number')(asset.dailyHigh, 2) + ' ' + _baseAsset + '/' + prefix + asset.symbol;
      } else {
        asset.dailyLow = '0';
        asset.dailyHigh = '0';
      }
      asset.lastPrice = $filter('number')(asset.lastPrice, 3) + ' ' + _baseAsset + '/' + prefix + asset.symbol;
      asset.feedText = $filter('number')(1 / asset.medianFeed, 2) + ' ' + _baseAsset + '/' + prefix + asset.symbol;
    }
